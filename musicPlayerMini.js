...
class GlobalMiniPlayer {
    constructor() {
        ...
        this.musicPlayerCore = null;
        this.init();
    }
    
    init() {
        // Check if we're on the jams page and if the music player exists
        if (window.musicPlayer && window.musicPlayer.core) {
            this.musicPlayerCore = window.musicPlayer.core;
            this.createMiniPlayerForOtherPages();
        } else if (window.MusicPlayerCore) {
            // Check if there's a standalone core player instance
            this.checkForCorePlayer();
        } else {
    ...
    checkForCorePlayer() {
        const checkInterval = setInterval(() => {
            // Look for any global core player instance
            if (window.backgroundPlayer) {
                this.musicPlayerCore = window.backgroundPlayer;
                this.createMiniPlayerForOtherPages();
    ...
    checkForMusicPlayer() {
        const checkInterval = setInterval(() => {
            if (window.musicPlayer && window.musicPlayer.core) {
                ...
                this.createMiniPlayerForOtherPages();
                clearInterval(checkInterval);
            } else if (window.backgroundPlayer) {
                this.musicPlayerCore = window.backgroundPlayer;
                this.createMiniPlayerForOtherPages();
                clearInterval(checkInterval);
            }
    ...
    createMiniPlayerForOtherPages() {
        ...
        if (window.location.pathname.includes('/jams/') || window.location.pathname.includes('/rabbitwine/jams/') || this.isCreated) {
            return;
        }
        ...
        this.updateMiniPlayerFromCorePlayer();
    ...
    togglePlayPause() {
        // Use the core player if available
        if (this.musicPlayerCore && this.musicPlayerCore.togglePlayPause) {
            this.musicPlayerCore.togglePlayPause();
        } else {
    ...
    goToPlayer() {
        // Navigate to jams player page - handle different path contexts
        const currentPath = window.location.pathname;
        const isOnRedscientist = window.location.hostname.includes('redscientist.com');
        let jamsPagePath;
        
        if (isOnRedscientist) {
            // On redscientist.com, use absolute path
            jamsPagePath = '/rabbitwine/jams/index.htm#player';
        } else if (currentPath.includes('/rabbitwine/')) {
            // Already in rabbitwine directory tree
            if (currentPath.includes('/jams/')) {
                // Already in jams directory
                jamsPagePath = './index.htm#player';
            } else {
                // In rabbitwine but not jams
                jamsPagePath = './jams/index.htm#player';
            }
        } else if (currentPath === '/' || currentPath.endsWith('/index.htm')) {
            // From a root directory
            jamsPagePath = './rabbitwine/jams/index.htm#player';
        } else {
            // From subdirectory, go up and find rabbitwine
            jamsPagePath = '../rabbitwine/jams/index.htm#player';
        }
    ...
    sendActionToJamsPage(actionType) {
        try {
            // Only try to send action to jams page if we're actually on the jams page
            const currentPath = window.location.pathname;
            const isOnJamsPage = currentPath.includes('/jams/') || currentPath.includes('/rabbitwine/jams/');
            
            if (!isOnJamsPage) {
                // We're not on the jams page, so handle locally
                return false;
            }
            
            // We're on the jams page - check if there's an active music state
            const currentState = localStorage.getItem('jamsMusicState');
            if (currentState) {
                const action = {
                    action: actionType,
    ...
    clearPlaylist() {
        ...
        if (this.musicPlayerCore && this.musicPlayerCore.clearPlaylist) {
            this.musicPlayerCore.clearPlaylist();
            // Hide the mini player immediately after clearing
            this.updateMiniPlayerDisplay({ hasPlaylist: false });
    ...
    updateMiniPlayerFromCorePlayer() {
        // Try to get music state from core player or localStorage
        let musicState = null;
        
        if (this.musicPlayerCore) {
            // Direct access to core player
            const state = this.musicPlayerCore.getCurrentState();
            musicState = {
    ...
    updateMiniPlayerDisplay(musicState) {
        ...
        const shouldShow = musicState.hasPlaylist && !window.location.pathname.includes('/jams/') && !window.location.pathname.includes('/rabbitwine/jams/');
...
document.addEventListener('DOMContentLoaded', () => {
    if (!window.location.pathname.includes('/jams/') && !window.location.pathname.includes('/rabbitwine/jams/')) {
